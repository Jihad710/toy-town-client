...
const Home = () => {
  ...
  useEffect(() => {
    ...
    return (
        <div className="flex h-full w-full items-center justify-center mt-7">
        <div className="w-full max-w-[1200px]">
          <ul  className="group flex flex-col gap-3 h-full md:h-[640px] md:flex-row md:gap-[1.5%]">
            {persons.map((person, index) => (
              <li
                onClick={() => setActiveItem(index)}
                aria-current={activeItem === index}
                className={classNames(
                  "relative cursor-pointer w-full md:w-[8%] md:first:w-[1%] md:last:w-[1%] md:[&[aria-current='true']]:w-[48%]",
              ...
              >
                <div className="relative h-full w-full overflow-hidden rounded-2xl bg-gray-300">
                  ...
                  <div
                    className={classNames(
                      "left-8 top-8 w-full md:w-[590px] p-4 transition-[transform,opacity] md:absolute md:p-0",
